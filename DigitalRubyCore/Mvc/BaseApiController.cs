...
namespace FeatureFlags.Core.Mvc
{
	...
	public abstract class BaseApiController : Controller
	{
		...
		public override void OnActionExecuting(ActionExecutingContext context)
		{
			...
			{
				if (kv.Value is not null && kv.Value is Contract.BaseRequest request)
				{
		...
		private void IdentifyRequest<T>(T request) where T : Contract.BaseRequest, new()
		{
			if (User.Identity is not null && User.Identity.IsAuthenticated)
			{
				var idClaim = User.Claims.FirstOrDefault(c => c.Type == ClaimTypes.NameIdentifier);
				if (idClaim is not null &&
					long.TryParse(idClaim.Value, NumberStyles.None, CultureInfo.InvariantCulture, out long userId))
				{
