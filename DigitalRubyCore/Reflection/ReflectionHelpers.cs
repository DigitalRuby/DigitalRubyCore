namespace FeatureFlags.Core.Reflection;
...
#nullable restore
	...
	public static IReadOnlyCollection<Type> GetAllTypes(string? namespaceFilter = null)
	{
		if (allTypes != null)
		{
			return allTypes;
		...
		foreach (Assembly a in GetAllAssemblies().Where(a =>
			string.IsNullOrWhiteSpace(namespaceFilter) || // no filter, grab the types
			a.FullName is null || // no full name, grab the types
			a.FullName.StartsWith(namespaceFilter, StringComparison.OrdinalIgnoreCase))) // assembly name starts with prefix, grab the types
		{
			try
			{
	...
	/// <param name="namespaceFilter">Namespace filter or null for none</param>
	/// <returns>Type or null if none found</returns>
	public static Type? GetType(string fullName, string? namespaceFilter = null)
	{
		Type? type = typeCache.GetOrAdd(fullName, _key =>
		{
			...
			foreach (Assembly assembly in ReflectionHelpers.GetAllAssemblies().Where(a =>
				string.IsNullOrWhiteSpace(namespaceFilter) || // no filter, grab the types
				a.FullName is null || // no full name, grab the types
				a.FullName.StartsWith(namespaceFilter, StringComparison.OrdinalIgnoreCase)))
			{
